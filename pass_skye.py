...
def play(board:List[List[int]], choices:List[int], player:int, memory:Any) -> Tuple[int, Any]:
    ...
    for col in choices:
        ...
        test_board[col].append(player)
        if count_winning_moves(test_board, player, board_size) >= 2:
            return col, memory # This move creates a fork, a very powerful offensive play.

    
    for col in choices:
        test_board = [c[:] for c in board]
        test_board[col].append(opponent)
        if count_winning_moves(test_board, opponent, board_size) >= 2:
            return col, memory # We must block this column to prevent the opponent's fork.
